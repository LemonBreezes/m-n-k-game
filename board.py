...
class Board():
    def __init__(self,
        ...
        self.board = [[0 for _ in range(num_columns)] for _ in range(num_rows)] if board is None else board
    ...
    def is_point_marked_by_player(self, x, y, player_id):
        return 0 <= x < self.num_rows and 0 <= y < self.num_columns and self.board[x][y] == player_id

    def is_point_marked(self, move):
        x, y = move
        return 0 <= x < self.num_rows and 0 <= y <= self.num_columns and self.board[x][y] != 0
