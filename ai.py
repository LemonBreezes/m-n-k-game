...
class AI:
    def __init__(
        ...
        def num_possible_states(num_rows, num_columns, num_players):
            res = 0
    ...
    def get_random_move(self, board):
        while True:
            ...
            if board.is_point_marked(move):
                continue
            return move

    def get_possible_next_states(self, board, curr_player):
        possible_next_states = [
            None for x, y in product(range(board.num_rows), range(board.num_columns))
        ]
    ...
    def get_optimal_move(self, board):
        ...
        scores = [
            self.score_state(state, prev_player=self.player_id)
            if state is not None
            else (-inf)
            for state in possible_next_states
        ]
        # print("Minimax scores: {scores}".format(p_id=self.player_id, scores=scores))
